...
export class CreateTokenStandardTransferRequest {
    /**
    ...
    */
    'expiresAt': number;
    /**
    * Tracking id to support exactly once submission. Once submitted, all successive calls with the same tracking id will get rejected with a 409 or 429 status code unless the command fails and the transfer did not get created. Clients should create a fresh tracking id when they try to create a new transfer. If that command submission fails with a retryable error or the application crashed and got restarted, successive command submissions must reuse the same tracking id to ensure they don\'t create the same transfer multiple times. 
    */
    'trackingId': string;
    ...
    static getAttributeTypeMap() {
        return CreateTokenStandardTransferRequest.attributeTypeMap;
    }
